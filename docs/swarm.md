...
### Prerequisites becoming ready

Kanbot 0.9.11 uses the existing task.updated inbox receipt to queue an assigned todo task after all its dependencies finish. It checks the original requester against the saved sender permissions, then checks ownership, prerequisites and permissions again before claiming. Repeated receipts, existing managed jobs and uncertain execution do not start duplicate work. Blocked/review tasks require reconciliation. Worker pause, mode and root budgets still apply; the receipt never starts a stopped engine.
