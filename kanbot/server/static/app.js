...
const S = {
  ...
  runners: [],
  workflows: [], workflowTemplates: [], workflowById: {}, distillAvailable: false,
  openCardId: null,
...
function openWorkflowsModal() {
  ...
  // If a build/draft in THIS session generated playbooks you haven't saved yet,
  // surface them instead of losing the (expensive) work — resume, don't re-run.
  // (Survival across reloads comes from the server-side DB cache, not the browser.)
  if (S.build && (S.build.made || []).length) {
    ...
    banner.appendChild(el('span', 'wf-resume-t', `✨ ${m} playbook${m === 1 ? '' : 's'} ${verb}, not yet saved`));
    ...
    const drop = el('button', 'btn ghost small', 'discard'); drop.onclick = () => { S.build = null; openWorkflowsModal(); };
